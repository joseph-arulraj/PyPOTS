...
class _DEARI(nn.Module):
    ...
    def forward(self, inputs: dict, training: bool = True) -> dict:
        ...
        f_predications = torch.softmax(f_logits, dim=1)
        b_predications = torch.softmax(b_logits, dim=1)
        ...
        if training:
            ...
            f_classification_loss = F.nll_loss(torch.log(f_predications), inputs["labels"])
            b_classification_loss = F.nll_loss(torch.log(b_predications), inputs["labels"])
